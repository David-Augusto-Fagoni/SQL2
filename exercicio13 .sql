...
--em viagens, de uma dada empresa, ordenado pela distância 

SELECT c.marca, c.modelo, SUM(v.distanciaPercorrida) distancia
FROM carro c, viagem v, empresa e
WHERE c.id = v.idCarro AND e.id = c.idEmpresa
...
SELECT e.nome, SUM(v.distanciaPercorrida) distancia
...
SELECT e.nome, SUM(v.distanciaPercorrida) distancia, CAST(AVG(v.distanciaPercorrida)AS decimal (10, 2)) media
FROM empresa e, carro c, viagem v
WHERE c.id = v.idCarro AND e.id = c.idEmpresa
GROUP BY e.nome
...
SELECT e.nome, MAX(v.distanciaPercorrida) maior
FROM empresa e, carro c, viagem v
WHERE c.id = v.idCarro AND e.id = c.idEmpresa
GROUP BY e.nome
...
SELECT e.nome, COUNT(c.id) carro
...
SELECT e.nome
FROM empresa e LEFT JOIN carro c ON e.id = c.idEmpresa
WHERE c.id IS NULL

--7) Consultar Marca e modelos dos carros que não fizeram viagens
SELECT c.modelo, c.marca
FROM carro c LEFT JOIN viagem v ON c.id = v.idCarro
...
SELECT COUNT(c.id) qnt, c.marca,c.modelo,e.nome
FROM empresa e, carro c, viagem v
WHERE e.id = c.idEmpresa AND v.idCarro = c.id
GROUP BY e.nome, c.marca, c.modelo
ORDER BY e.nome ASC, qnt DESC
...
CASE WHEN (v.distanciaPercorrida < 1000) THEN
	'R$ '+CAST((LEFT(v.distanciaPercorrida/1000,1)*10)AS varchar) 
ELSE
	'R$ '+CAST((LEFT(v.distanciaPercorrida/1000,1)*15)AS varchar) 
END
FROM empresa e, carro c, viagem v
WHERE e.id = c.idEmpresa AND v.idCarro = c.id
